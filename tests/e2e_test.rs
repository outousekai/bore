...
async fn spawn_server(secret: Option<&str>) {
    tokio::spawn(Server::new(None,1024..=65535, secret).listen());
...
async fn invalid_address() -> Result<()> {
    ...
    async fn check_address(to: &str, use_secret: bool) -> Result<()> {
        match Client::new("localhost", 5000, to, 0, use_secret.then_some("a secret"),None).await {
...
fn empty_port_range() {
    let min_port = 5000;
    let max_port = 3000;
    let _ = Server::new(None,min_port..=max_port, None);
}
